(function (global, factory) {
  ...
  const composeArgs = (args, specificSchema) => {
    ...
    Object.keys(argsForSchema).forEach(arg => {
      ...
      composedArgs.push("".concat(arg, ":").concat(argValue));
    });
    return composedArgs.length > 0 ? " (".concat(composedArgs.toString(), ")") : '';
  };
  /**
  ...
  const stringifyField = fields => {
    ...
    Object.keys(fields).forEach(field => {
      ...
      if (typeof fields[field] === 'object' && fields[field] !== null) {
        composedFields.push("".concat(field, " {").concat(stringifyField(fields[field]), "}"));
      } else {
  ...
  const composeQuery = (schema, options) => {
    ...
    Object.keys(schema).forEach(schemaItem => {
      query += "".concat(schemaItem).concat(composeArgs(args, schemaItem), " {\n      ").concat(composeFields(schema[schemaItem]), "\n    }");
    });
    return "{\n    ".concat(query, "\n  }");
  };
  /**
